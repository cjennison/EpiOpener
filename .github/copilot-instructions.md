...
- EMOJI USAGE IS FORBIDDEN in code comments, logs, and commit messages.
- Code comments must be professional and production-ready. DO NOT write educational comments like "learning opportunity", "tutorial", "example", or "demonstration". This is a client project, not a teaching tool.
- Comments should explain WHY, not WHAT. The code itself should be self-documenting for WHAT it does.
- Configuration values (like container names, database names, API endpoints) belong in code constants, NOT environment variables, unless they vary by environment (dev/staging/prod).
